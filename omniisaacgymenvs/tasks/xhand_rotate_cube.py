...
from omniisaacgymenvs.robots.articulations.xhand_R import XHandR
from omniisaacgymenvs.robots.articulations.views.xhand_R_view import XHandRView
...
class XHandRotateCubeTask(InHandManipulationTask):
    def __init__(
        self, 
        name, 
    ...
    def get_starting_positions(self):
        ...
        self.hand_start_orientation = torch.tensor([1, 0.70711, 0.70711, 0], device=self.device)
    ...
    def get_hand(self):
        ...
        inpsire_hand = XHandR(
            prim_path=self.default_zero_env_path + "/xhand_right", 
            name="xhand_right", 
        ...
        self._sim_config.apply_articulation_settings(
            "xhand", get_prim_at_path(inpsire_hand.prim_path), self._sim_config.parse_actor_config("xhand")
        )
        # inpsire_hand.setins(stage=self._stage, prim=inpsire_hand.prim)
        
    def get_hand_view(self, scene):
        return XHandRView(prim_paths_expr="/World/envs/.*/xhand_right", name="xhand_right_view")
    ...
    def compute_full_observations(self, no_vel=False):
        if no_vel:
            ...
            self.obs_buf[:, 37:40] = self.goal_pos
            self.obs_buf[:, 40:44] = self.goal_rot
