...
from omniisaacgymenvs.robots.articulations.realman import Realman
from omniisaacgymenvs.robots.articulations.views.realman_view import RealmanView

import torch
import numpy as np


class RealmanFollowTargetTask(RLTask):
    def __init__(
        self,
    ...
    def set_up_scene(self, scene) -> None:
        ...
        self._realmans = RealmanView(
            prim_paths_expr="/World/envs/.*/realman/rm_75_6f_description", name="realman_view"
        )

        # register the ArticulationView object to the world, so that it can be initialized
        scene.add(self._realmans)
    ...
    def post_reset(self):
        ...
        self.end_effectors_init_pos, self.end_effectors_init_rot = (
            self._realmans._hands.get_world_poses()
        )
    ...
    def reset_index(self, env_ids):
        ...
        self._realmans.set_joint_positions(positions=dof_pos, indices=indices)
        self._realmans.set_joint_velocities(velocities=dof_vel, indices=indices)
    ...
    def get_randomized_DOF_positions(self, indices):
        ...
        dof_limits = self._realmans.get_dof_limits()
        ...
        self.franka_dof_default_pos = self._realmans._default_joints_state.positions
        ...
        dof_pos = torch.zeros((self._num_envs, self._realmans._num_dof), device=self._device)
    ...
    def get_randomized_DOF_velocities(self, indices):
        ...
        self.franka_dof_default_vel = self._realmans._default_joints_state.velocities
        ...
        dof_vel = torch.zeros((self._num_envs, self._realmans._num_dof), device=self._device)
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        # print(actions)
        self.actions = torch.zeros((self._num_envs, self._realmans._num_dof), dtype=torch.float, device=self.device)
        self.actions[:, :self._num_actions] = actions
        self.franka_dof_target = torch.zeros((self._num_envs, self._realmans._num_dof), dtype=torch.float, device=self.device)
        ...
        self._realmans.set_joint_position_targets(self.franka_dof_target, indices=env_ids_int32)

        # got franka dot limites
        dof_limits = self._realmans.get_dof_limits()
        self.franka_dof_lower_limits = dof_limits[0, :, 0].to(device=self._device)
        self.franka_dof_upper_limits = dof_limits[0, :, 1].to(device=self._device)
        end_effectors_pos, end_effectors_rot = self._realmans._hands.get_world_poses()
    ...
    def get_observations(self) -> dict:
        ...
        # observations = {self._realmans.name: {"obs_buf": self.obs_buf}}

        # real observations
        arm_joint_indices = [0, 1, 2, 3, 4, 5, 6]
        # 7: franka joints position (action space)
        self.arm_dof_pos = self._realmans.get_joint_positions(joint_indices=arm_joint_indices)
        # 7: franka joints velocity (action space)
        self.arm_dof_vel = self._realmans.get_joint_velocities(joint_indices=arm_joint_indices)
    ...
    def get_reset_target_new_pos(self, n_reset_envs):
        ...
        new_pos[:, 0] = new_pos[:, 0] * 0.10 + 0.30 * torch.sign(new_pos[:, 0])
        new_pos[:, 1] = new_pos[:, 1] * 0.10 + 0.30 * torch.sign(new_pos[:, 1])
        new_pos[:, 2] = torch.abs(new_pos[:, 2] * 0.4) + 0.15
        return new_pos
    ...
    def get_realman(self):
        # add a single robot to the stage
        realman = Realman(
            prim_path=self.default_zero_env_path + "/realman",
            name="realman",
        ...
        self._sim_config.apply_articulation_settings(
            "realman/rm_75_6f_description",
            get_prim_at_path(realman.prim_path),
            self._sim_config.parse_actor_config("realman"),
        )
        realman.set_realman_properties(stage=self._stage, prim=realman.prim)
